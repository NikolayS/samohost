...
export function buildDeployScript(app: AppRecord, target: DeployTarget): string {
  ...
  if (app.kind === "static") {
    push(
      ...
      'SAMOHOST_OLD_ROUTE_HOST=""',
      'SAMOHOST_OLD_ROUTE_ADDRESS=""',
      ...
      'if [[ "$SAMOHOST_OLD_VHOST_PRESENT" == "1" ]]; then',
      '  if ! IFS=$\'\t\' read -r SAMOHOST_OLD_ROUTE_SCHEME SAMOHOST_OLD_ROUTE_HOST SAMOHOST_OLD_ROUTE_ADDRESS < <(python3 - "$SAMOHOST_VHOST_BACKUP" <<\'PY\'',
      "import re",
      "import sys",
      "text = open(sys.argv[1], encoding='utf-8').read()",
      "match = re.search(r'^\\s*((?:(https?)://)?([a-z0-9.-]+)(?::(80|443))?)\\s*\\{', text, re.MULTILINE)",
      "if not match: raise SystemExit('cannot parse prior static route address')",
      "raw, explicit_scheme, host, port = match.groups()",
      "scheme = explicit_scheme or ('http' if port == '80' else 'https')",
      "print(scheme, host, f'{scheme}://{host}', sep='\\t')",
      "PY",
      '  ); then echo "cannot snapshot prior static route identity" >&2; exit 1; fi',
      '  [[ -n "$SAMOHOST_OLD_ROUTE_SCHEME" && -n "$SAMOHOST_OLD_ROUTE_HOST" && -n "$SAMOHOST_OLD_ROUTE_ADDRESS" ]] || { echo "prior static route identity is incomplete" >&2; exit 1; }',
      "fi",
  ...
  if (app.kind === "static") {
    push(
      ...
      '    rb_code=$(curl -s -k -H "Host: $SAMOHOST_OLD_ROUTE_HOST" -o /dev/null -w "%{http_code}" --max-time 10 "$SAMOHOST_OLD_ROUTE_SCHEME://127.0.0.1/" || echo 000)',
      '  elif [[ "$rb_ok" == "1" && "${SAMOHOST_OLD_VHOST_PRESENT:-0}" == "0" && ! -e ' + sq(staticMainSnippet!) + ' ]]; then',
      '    rb_code="200"',
      "  fi",
      ...
      '    echo "rollback restore/old-route health failed (HTTP $rb_code); candidate and rollback artifacts retained for recovery" >&2',
      "  fi",
      "  exit 1",
      "}",
      "",
