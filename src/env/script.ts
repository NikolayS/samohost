 ...
 * client-owned FQDN and reloads Caddy.  Pushed over SSH by `domain add`.
 *
 * Snippet path: `/etc/caddy/sites.d/10-domain-<label>.caddy`
 * (`10-` sorts after the `00-main-*` main-env vhost; label = fqdn dots→dashes).
 *
 * Serving posture mirrors the main-env vhost:
 *   - node apps: `reverse_proxy localhost:<port>` (port from healthUrl)
 *   - static apps: `root * <appDir>; file_server`
 * Both use `tls internal` — CF Full-mode terminates the edge cert; the origin
 * self-signed cert is never seen by the browser.
 *
 * The script runs `caddy validate` before `systemctl reload caddy` as a
 * safety net.  Relies on the same NOPASSWD sudoers grants that env-create
 * scripts use (already present after `env plan --host-prep` + `app bootstrap`).
 *
 * @throws if `app.mainHost` is absent and `fqdn` does not pass isValidMainHost
 *   (the FQDN is embedded in a root-run script — fail closed on injection risk).
 */
export function buildCustomDomainVhostScript(
  ...
  return [
    "#!/usr/bin/env bash",
    `# samohost custom-domain vhost: ${fqdn} → ${app.name}`,
    ...
    "",
    `# Validate config before reloading to catch syntax errors early.`,
 ...
 * client-owned FQDN and reloads Caddy.  Pushed over SSH by `domain rm`.
 */
export function buildCustomDomainVhostRemoveScript(fqdn: string): string {
  ...
  return [
    "#!/usr/bin/env bash",
    `# samohost custom-domain vhost removal: ${fqdn}`,
