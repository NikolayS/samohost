...
const SYNC_CLONE_GLOBALS_FN_LINES: string[] = [
  ...
  "  #      fail with 'permission denied for schema public'. Scoped to app roles",
  "  #      only via grep -Ff: names already ^[a-z_][a-z0-9_]*$ and unique in",
  "  #      the app role set so substring collisions are not possible in practice.",
  '  sudo -u postgres /usr/bin/psql -At -d "$prod_db" -c "SELECT \'GRANT \'||priv||\' ON SCHEMA \'||quote_ident(n.nspname)||\' TO \'||quote_ident(r.rolname)||\';\' FROM pg_namespace n CROSS JOIN pg_roles r CROSS JOIN (VALUES(\'USAGE\'),(\'CREATE\')) pv(priv) WHERE n.nspname NOT IN (\'pg_catalog\',\'information_schema\',\'pg_toast\') AND r.rolname NOT LIKE \'pg\\_%\' AND r.rolname <> \'postgres\' AND has_schema_privilege(r.rolname, n.nspname, priv)" 2>/dev/null \\',
  '    | grep -Ff "$scoped_roles" \\',
  '    | PGPASSWORD="$SAMOHOST_DB_PASSWORD" psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -p "$SAMOHOST_DB_PORT" -U samohost_env -d "$prod_db" -f - >/dev/null 2>&1 || apply_failures=$((apply_failures+1))',
  '  rm -f "$scoped_roles"',
